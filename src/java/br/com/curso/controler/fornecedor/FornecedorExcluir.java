 ...
 * @author johat
 */
@WebServlet(name = "FornecedorExcluir", urlPatterns = {"/FornecedorExcluir"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       int idFornecedor = Integer.parseInt(request.getParameter("idFornecedor"));
        ...
        try {
            ...
            if(dao.excluir(idFornecedor)){
                response.getWriter().write("1");
            }else{
                response.getWriter().write("0");
            }
        } catch (Exception e){
            System.out.println("Problemas na Servelet Excluir Fornecedor!Erro: " + e.getMessage());
            e.printStackTrace();
        }
    }
